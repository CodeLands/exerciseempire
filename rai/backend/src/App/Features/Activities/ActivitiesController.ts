...
export class ActivitiesController {
  ...
  public listActivityStats = async (req: Request, res: Response) => {
    const { user_id } = req.query;

    const validation = this.activitiesValidator.validateUserId(Number(user_id));
    if (!validation.success) {
      return res.json({
        success: false,
    ...
    const result = await this.activitiesRepository.listActivityStats(validation.data.user_id);
    if (result.status === RepositoryResultStatus.dbError)
      return res.json({
        success: false,
        errors: ["Database Error!"],
    ...
    return res.json({
      success: true,
      message: "Getting all activity stats for user",
      data: result.data,
    });
  }

  public getActivityDetails = async (req: Request, res: Response) => {
    ...
    const result = await this.activitiesRepository.getActivityDetails(Number(activity_id));
    if (result.status === RepositoryResultStatus.dbError)
      return res.json({
        success: false,
        errors: ["Database Error!"],
    ...
    if (result.status === RepositoryResultStatus.failed) {
      return res.json({
        success: false,
        message: "No activity details found",
      });
    }
    return res.json({
      success: true,
